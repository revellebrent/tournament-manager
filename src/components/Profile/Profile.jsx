...
import "./Profile.css";
import FileUploaad from "../FileUpload/FileUpload";
import { useAuth } from "../../context/AuthContext";
import { ensureUser, listUsersByRole, addDocuments, listDocumentsByOwner, shareDocument } from "../../utils/db";
...
export default function Profile() {
 ...
 function handleSend(docId) {
  if (!toCoach) return;
  shareDocument({ fromEmail: email, toEmail: toCoach, documentId: docId, message: "Player card" });
  alert("Sent to coach");
 }
            ...
            {docs.map(d => (
              <li key={d.id} className="profile__doc">
                ...
                <div className="profile__docactions">
                  <select className="profile__select" value={toCoach} onChange={(e)=>setToCoach(e.target.value)}>
                    {coaches.map(c => <option key={c.email} value={c.email}>{c.name} ({c.email})</option>)}
                  </select>
                  <button className="button" type="button" onClick={()=>handleSend(d.id)}>Send to coach</button>
                </div>
...
function Preview({ mime, dataUrl }) {
  if (mime === "image/jpeg") return <img className="profile__preview" src={dataUrl} alt="player card" />;
  if (mime === "application/pdf") return <object className="profile__preview" data={dataUrl} type="application/pdf" aria-label="Player card PDF" />;
  return null;
}